...
class Writer {
  ...
  getIndexContent () {
    ...
    const title = projectInfo.hasOwnProperty('title') ? projectInfo.title :'Loading...';
    const description = projectInfo.hasOwnProperty('description') ? projectInfo.description : 'API Documentation';
  ...
  createSingleFile () {
    ...
    const prismCss = this.fs.readFileSync(this.findAsset('prismjs/themes/prism-tomorrow.css'), 'utf8');
      const mainCss = this.fs.readFileSync(__dirname + '/../template/src/css/main.css', 'utf8');
